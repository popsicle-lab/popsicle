# 架构辩论模拟器（Architecture Debate Simulator）

通过多角色辩论模拟，在重大架构决策前充分暴露盲点、探索方案空间。用户以可配置的置信度参与讨论，产出高质量架构方案和 ADR 草稿。
...
1. **项目配置**（优先）：读取 `docs/arch-debate/roles.md` 获取领域角色定义，读取 `docs/arch-debate/product-mapping.md` 获取产品-角色映射关系
2. **内置默认**（兜底）：若项目配置不存在，使用 `references/default-roles.md` 中的 5 个通用角色

**角色选择流程：**
- 根据用户指定的产品/模块，从映射表推荐 4-6 个角色
- 向用户展示推荐阵容，允许增减调整
...
1. **ADR 草稿** — 可直接交给 `adr` skill 格式化归档
...
arch-debate（本 skill）
    │ 产出：ADR 草稿 + 决策矩阵
    │
    ├─→ 来自 product-debate？→ 回到 feature-submission 继续入库
    │
    └─→ 独立启动？→ 建议启动 `feature-submission` 正式入库
            │ 复用本次辩论的 ADR 草稿和决策矩阵
...
1. **如果是 `product-debate` 引导过来的**：
   > "架构方案已确定。可继续调用 `feature-submission` 正式入库，本次辩论的 ADR 草稿将与之前的 PRD 草稿一并纳入 Feature Spec。"

2. **如果是独立启动的架构讨论**：
   > "架构方案已确定。如需正式入库为技术需求，可调用 `feature-submission`（类型选 `technical`）生成 Feature Spec 并更新全景图。本次辩论的 ADR 草稿将被自动复用。"

3. **如果仅为技术探索、尚未到入库阶段**：
   > 不主动建议入库，仅产出 ADR 草稿和会议纪要供后续参考。
...
| 文件路径 | 用途 | 缺失时行为 |
|---------|------|-----------|
| `docs/arch-debate/roles.md` | 项目领域角色定义 | 使用内置通用角色 |
| `docs/arch-debate/product-mapping.md` | 产品-角色默认映射 | 用户手动选择角色 |
...
- **`references/default-roles.md`** — 5 个内置通用角色定义（ARCH, SEC, PERF, OPS, MAINT）
- **`references/confidence-modes.md`** — 5 级置信度的详细行为规则和交互差异
- **`references/debate-phases.md`** — 4 个辩论阶段的详细说明、参与者、暂停点设计
- **`references/output-templates.md`** — ADR 草稿、会议纪要、决策矩阵的产出模板
