...
ADR documents are co-located with Feature Specs when feature-scoped, or placed in the cross-cutting directory:

- **Feature-scoped**: `docs/features/{type}/{module}/{feature-name}/{ID}-{topic}-adr.md` (e.g., `docs/features/product/vera/event-reporting/VR-EVT-001-storage-choice-adr.md`)
- **Cross-cutting**: `docs/features/cross-cutting/{topic-dir}/{NNNN}-{topic}-adr.md` (e.g., `docs/features/cross-cutting/multi-tenant/0013-new-decision-adr.md`). If the ADR doesn't belong to an existing topic group, place it directly in `docs/features/cross-cutting/`.

Before creating a new ADR:
1. Ask the user if this ADR is associated with an existing Feature Spec (Feature ID)
2. If feature-scoped: save in the Feature Spec's subdirectory `docs/features/{type}/{module}/{feature-name}/{ID}-{topic}-adr.md`
3. If cross-cutting: read `docs/features/cross-cutting/INDEX.md` to determine the next sequential number, then save as `{NNNN}-{topic}-adr.md`

After creating the ADR:
- **Feature-scoped**: update the Feature Spec's "六、关联文档" section with a link to the ADR
- **Cross-cutting**: append a new row to the ADR index table in `docs/features/cross-cutting/INDEX.md`
...
- Feature ID: [e.g., VR-EVT-001 or "cross-cutting"]
- Feature Spec: [Relative link to the associated Feature Spec, if any]
...
ADRs are co-located with their Feature Specs. Cross-cutting ADRs live in a shared directory:

```
docs/features/
├── cross-cutting/
│   ├── INDEX.md                                    # ADR/PRD index for cross-cutting items
│   ├── multi-tenant/                               # grouped by feature topic
│   │   ├── multi-tenant-deployment-prd.md
│   │   └── 0008-multi-tenant-architecture-adr.md
│   ├── storage-architecture/
│   │   └── 0003-hybrid-storage-adr.md
│   └── 0012-standalone-decision-adr.md             # ungrouped ADRs stay at root
├── product/vera/
│   └── event-reporting/                            # Feature subdirectory
│       ├── VR-EVT-001-event-reporting.md           # Feature Spec
│       ├── VR-EVT-001-event-reporting-prd.md       # Co-located PRD
│       └── VR-EVT-001-storage-choice-adr.md        # Co-located ADR
└── technical/nfr/
    └── write-throughput/                           # Feature subdirectory
        ├── NFR-PERF-001-write-throughput.md
        └── NFR-PERF-001-write-throughput-adr.md
```

### ADR Index

Cross-cutting ADRs are indexed in `docs/features/cross-cutting/INDEX.md`. Feature-scoped ADRs are referenced in their Feature Spec's "六、关联文档" section.

When creating a new cross-cutting ADR, always update `INDEX.md` with the new entry.

### Naming Convention

- **Feature-scoped ADR**: `{feature-name}/{FeatureID}-{topic}-adr.md` — stored in the feature's own subdirectory; the topic can differ from the feature name when the ADR addresses a specific sub-concern
- **Cross-cutting ADR**: `{topic-dir}/{NNNN}-{topic}-adr.md` — sequential numbering continues from existing INDEX.md
- Multiple ADRs per feature are allowed, each with a distinct `{topic}`, all within the same feature subdirectory
