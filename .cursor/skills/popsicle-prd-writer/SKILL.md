---
name: popsicle-prd-writer
description: 把辩论产出（或用户直接需求）打磨成可落地的 IDD 任务图产物——PRD overview（PRODUCT.md 顶层增量）+ N 份 task 文件（按 5 个用户旅程阶段归类）+ acceptance.intent 种子（单文件多 block）+ PDR 骨架。强制贯彻「以任务和意图为中心、告别功能罗列」的 AI 时代文档范式，质量评分 ≥ 90 才放行。 (from module: intent-coder)
---

> This skill is provided by the **intent-coder** module.

Perform the "prd-writer" step in the Popsicle pipeline.
...
- **Transitions**:
  ...
  - `scoring` → `review` via `pass` (guard: `has_sections:Core Intent,Problem Statement,File Manifest,User Intents Catalog,Intent Mapping;checklist_complete:Quality Checklist`)
...
popsicle prompt prd-writer --run <run-id> --related --format json

# Create the document
popsicle doc create prd-writer --title "<title>" --run <run-id>
