---
name: popsicle-product-debate
description: 多角色产品讨论模拟器——在写 PRD 之前用 4-6 个角色（PM / UXR / ENGLD / BIZ / ...）就一个 product slice 充分辩论方案空间，暴露盲点。**消费 fact-extractor 的事实基**作为辩论的 ground truth，**消费 project-init 的 product inventory** 作为讨论边界，产出 PRD 草稿 + 决策矩阵 + 辩论纪要，喂给下游的 prd-writer。 (from module: intent-coder)
---

> This skill is provided by the **intent-coder** module.

Perform the "product-debate" step in the Popsicle pipeline.
...
popsicle prompt product-debate --run <run-id> --related --format json

# Create the document
popsicle doc create product-debate --title "<title>" --run <run-id>
