---
name: popsicle-intent-consistency-check
description: 对项目内所有 .intent 文件跑 intent-lang 形式化验证（经 intent-validate tool / Z3），汇总 verified/failed/skipped 并产出一致性报告。skill 始终是「observe」——报告问题但不阻断 pipeline；「gate」（任一失败即拦合并）由 CI 直接用 intent-validate tool 的 exit code 实现，skill 负责给出量化的「何时开闸」退出判据（连续 N 次零失败才升级）。只验证逻辑一致性，不覆盖时间/性能/运行时约束。 (from module: intent-coder)
---

> This skill is provided by the **intent-coder** module.

Perform the "intent-consistency-check" step in the Popsicle pipeline.
...
- **Transitions**:
  ...
  - `review` → `reporting` via `revise`
  - `checking` → `reporting` via `results-collected`
  - `reporting` → `review` via `submit` (guard: `has_sections:Summary,Per-File Results,Failures,Skipped,Disposition,Gate Readiness;checklist_complete:检查清单`)

## Inputs (upstream dependencies)

- `formal-acceptance-intent` from skill `intent-spec-writer` (optional)
...
popsicle prompt intent-consistency-check --run <run-id> --related --format json

# Create the document
popsicle doc create intent-consistency-check --title "<title>" --run <run-id>
