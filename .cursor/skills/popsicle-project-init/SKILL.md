---
name: popsicle-project-init
description: 把一份遗留代码库 bootstrap 成 IDD-ready 仓库——初始化 popsicle、把 legacy 源码 pin 成 submodule、为每个 product 铺出 4 件套目录（PRODUCT / ARCHITECTURE / intents / decisions+proposals）、产出一份命名了 product 和首个切片的迁移计划 (from module: intent-coder)
---

> This skill is provided by the **intent-coder** module.

Perform the "project-init" step in the Popsicle pipeline.

## Workflow

- **Initial state**: `surveying`
- **Final state(s)**: `completed`
- **Transitions**:
  - `planning` → `scaffolding` via `approve` (guard: `has_sections:Repository Identity,Product Inventory,Legacy Source,First Migration Slice,Scaffolding Manifest;checklist_complete:Plan Checklist`) **⚠ requires human approval**
  - `planning` → `surveying` via `revise`
  - `surveying` → `planning` via `plan` (guard: `checklist_complete:Survey Checklist`)
  - `scaffolding` → `completed` via `confirm` (guard: `checklist_complete:Scaffolding Verification`) **⚠ requires human approval**
...
popsicle prompt project-init --run <run-id> --related --format json

# Create the document
popsicle doc create project-init --title "<title>" --run <run-id>
