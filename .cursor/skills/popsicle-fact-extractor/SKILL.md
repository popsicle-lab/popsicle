---
name: popsicle-fact-extractor
description: 对一份遗留代码库做逆向工程，产出结构化的事实基——dependency graph、public-API contracts、unsafe/risk 清单、tech-debt 清单——下游所有 IDD skill 都把它当作 ground truth 来引用 (from module: intent-coder)
---

> This skill is provided by the **intent-coder** module.

Perform the "fact-extractor" step in the Popsicle pipeline.
...
- **Transitions**:
  - `drafting` → `review` via `submit` (guard: `has_sections:Summary,Bounded Contexts,Domain Glossary,Risk Hotspots;checklist_complete:Extraction Checklist`)
  - `scanning` → `drafting` via `data-collected`
...
popsicle prompt fact-extractor --run <run-id> --related --format json

# Create the document
popsicle doc create fact-extractor --title "<title>" --run <run-id>
