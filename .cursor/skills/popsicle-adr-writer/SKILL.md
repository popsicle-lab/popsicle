---
name: popsicle-adr-writer
description: 把 rfc-writer 产出的 ADR 骨架（Status 为 Proposed）固化为正式 ADR（Accepted，此后不可变），并**解锁** contracts.intent 种子的 `[Awaiting ADR-XXXX]` 状态——交 intent-spec-writer 收紧合并。是产品侧「PDR 审批落地」在技术侧的对称把关者。**不重新发明 ADR 内容**（那是 rfc-writer），只做固化门 + 解锁 + 一致性核对。 (from module: intent-coder)
---

> This skill is provided by the **intent-coder** module.

Perform the "adr-writer" step in the Popsicle pipeline.

## Workflow

- **Initial state**: `reviewing`
- **Final state(s)**: `completed`
- **Transitions**:
  - `review` → `completed` via `approve` **⚠ requires human approval**
  - `review` → `reviewing` via `revise`
  - `reviewing` → `finalizing` via `gate-passed` (guard: `checklist_complete:Finalization Gate`)
  - `finalizing` → `review` via `submit` (guard: `has_sections:Summary,固化检查,解锁动作,Intent Impact 核对;checklist_complete:检查清单`)

## Inputs (upstream dependencies)

- `adr-skeleton` from skill `rfc-writer` (required)
- `rfc` from skill `rfc-writer` (optional)
- `contracts-intent-seed` from skill `rfc-writer` (optional)
- `arch-debate-record` from skill `arch-debate` (optional)
...
popsicle prompt adr-writer --run <run-id> --related --format json

# Create the document
popsicle doc create adr-writer --title "<title>" --run <run-id>
