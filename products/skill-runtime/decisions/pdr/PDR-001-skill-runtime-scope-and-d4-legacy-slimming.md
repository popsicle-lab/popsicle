# PDR-001 · skill-runtime scope & D4 legacy slimming

> **Status**: Accepted
> **Date**: 2026-06-08
> **Stage**: `debate` (intent-coder migration-bootstrap pipeline, run `f89529af-d8ce`)
> **Lineage**: supersedes `docs/rfc-workflow-only-core.md` (RFC D2) · aligns `intent-coder/ROADMAP.md §D4` · charter footnote = `ADR-001` (待 adr stage 起草)
> **Source artifact**: `.popsicle/artifacts/f89529af-…/skill-runtime-product-debate-scope--d4-legacy-slimming.product-debate.md` (popsicle-managed; full debate transcript)

本文是 popsicle-new 首切片 `skill-runtime` 的 **Product Decision Record**，由 intent-coder `product-debate` skill 在 `debate` stage 产出，并经用户在 setup / Phase 1 / Phase 2 / Phase 3 / Phase 4 五个暂停点全程审批 freeze。

下文为 debate-record body 直接提升（去 frontmatter），完整 metadata 见 source artifact。
