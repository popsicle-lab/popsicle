...
手机（或远程 Web）派活 + 本机 Daemon 自动跑 `popsicle` pipeline 并调起 Agent CLI；Server 只协调队列与状态，不执行 AI、不持有 API Key。
...
- **手机 App（Expo）**：`apps/mobile/` — 设置 / 派活 / 进度 / Run 详情批准（T-AR-0002–0004）
...
- [Onboarding](tasks/onboarding/) — Daemon 安装、首次派活（2 task）
- [Daily-Ops](tasks/daily-ops/) — 进度查看、远程审批（2 task）
