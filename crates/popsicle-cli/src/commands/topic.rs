...
pub fn execute(cmd: TopicCommand, format: &OutputFormat) -> Result<()> {
    match cmd {
        TopicCommand::Create {
            ...
            description,
            tags,
        } => create_topic(&name, &description, &tags, format),
        TopicCommand::List => list_topics(format),
...
fn list_topics(format: &OutputFormat) -> Result<()> {
    let layout = project_layout()?;
    let db = IndexDb::open(&layout.db_path())?;
    let topics = db.list_topics().context("Failed to list topics")?;
