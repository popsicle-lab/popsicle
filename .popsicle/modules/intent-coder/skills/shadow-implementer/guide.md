...
### verbatim 平移 vs shadow（措辞校正，#22）

「shadow / strangler-fig / in-shadow」语义是「legacy 仍主路径、新代码影子并行、
有流量对比」。**逐字节/逐行平移（verbatim）时这套措辞不成立**：新代码直接就是主路径，
没有影子并行，golden 也只是 characterization（快照自证，非 legacy↔new 差分）。
在 `equivalence-report` frontmatter 用 `migration_mode: verbatim` 显式声明，并在
coverage 报告里如实写「本切片为 verbatim 平移，无影子并行」，不要套用 strangler 叙事。
真正的 rewrite 才需要 shadow 并行 + legacy 录制回放差分（见 ROADMAP 的 `golden-capture`）。
