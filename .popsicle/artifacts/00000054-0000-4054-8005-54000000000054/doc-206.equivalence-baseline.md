...
status: active
title: PROJ-86 verify：make check
version: 1
---

# PROJ-86 verify：make check
...
|---|---|
| Slice | agent-runtime |
| Mode | tests（feature-delivery，无 legacy golden）|
| `make check` | PASS |
| agent-daemon tests | 8 passed |
| **equivalence_gate_pass** | true |

门禁：`make check`（fmt + clippy -Dwarnings + test）全绿 → pass。

## Golden Inventory

- [x] slice 已确认（agent-daemon cursor-agent adapter）
- [x] 以 `cargo test -p agent-daemon` + 全 workspace `make check` 替代 golden
- [x] 追溯到 ADR-001 §5 adapter 与 contracts `DaemonSubprocessInvokesPopsicle`
- [x] 无 divergence
...
```
make check — exit 0
cargo test -p agent-daemon — 8 passed
```

## 检查清单

- [x] make check 已实跑
- [x] adapter dry-run 单测通过
- [x] 无未登记行为漂移
