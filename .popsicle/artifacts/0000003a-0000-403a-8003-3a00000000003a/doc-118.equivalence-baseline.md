...
ADR-029 为 **CLI 内部 taxonomy 重命名**，不改变 legacy submodule 对外契约。旧 pipeline 名经 `canonical_pipeline_name` alias 解析，Issue create/start、门禁与 golden 测试行为与 PROJ-58 基线一致。无新增 divergence。

## Golden 清单

| # | 场景 | 命令/测试 | 预期 |
|---|---|---|---|
| G1 | Issue type 默认 pipeline 可加载 | `golden_010_issue_type_default_pipelines_are_bundled` | pass |
| G2 | alias 解析 slice-delivery → migration-slice-delivery | `pipeline_taxonomy::tests` | pass |
| G3 | slice-delivery 门禁（create/start） | `local_workspace` slice_delivery_* | pass |
| G4 | fix-regression 门禁（原 bugfix） | `pipeline_gate::tests` | pass |
| G5 | workflow catalog 列出 canonical pipelines | `workflow_catalog` tests | pass |
| G6 | skill-runtime 加载 migration-slice-delivery YAML | `golden_003_load_slice_delivery_pipeline` | pass（路径已更新） |
| G7 | dogfood live pipeline 解析 | `intent_coder_resolve` tests | pass |

## 运行结果

```
make check  →  pass（2026-06-23）
```

本 slice 不修改 `legacy/popsicle/` 行为面；golden 链无需新 baseline 快照。

## Traceability

| 条目 | 说明 |
|---|---|
| ADR-029 | Pipeline 命名 taxonomy |
| PROJ-59 | 实现 Issue |
| T-CU-0017 | workflow catalog UI 域分组 |
...
- [x] ≥5 golden 场景 pass（见上表 G1–G7）
- [x] alias 不破坏既有 Issue/CLI 脚本
- [x] `make check` 全绿
...
- [x] Golden 清单已列全
- [x] 运行结果已记录
- [x] Traceability 已关联 ADR-029
- [x] Divergence 表为空（无需 Accepted 豁免）
