...
id: doc-223
pipeline_run_id: 0000005c-0000-405c-8005-5c00000000005c
status: active
title: PROJ-93 桌面 UI 启动 Daemon
version: 1
---

# PROJ-93 桌面 UI 启动 Daemon
## Scope

扩展 T-AR-0001：Settings → Agent Runtime 可启动/停止本机 Daemon，无需手动开终端 `--foreground`。

## Implementation

| 路径 | 变更 |
|---|---|
| `crates/cli-ux/src/daemon.rs` | `daemon_start_background` 后台 spawn；PID 文件 `~/.popsicle/daemon/pid`；`daemon_stop` 发 SIGTERM；`server_client_for_daemon()` 读 `agent-runtime.json`；CLI `--background` |
| `crates/cli-ux/src/ui/runtime_commands.rs` | `daemon_start_cmd` / `daemon_stop_cmd` |
| `crates/cli-ux/src/ui/dto.rs` | `DaemonControlResultDto`；`DaemonStatusDto` 增 `poll_running`/`pid`/`log_path` |
| `ui/src/components/AgentRuntimeSection.tsx` | 启动/停止按钮 |
| `ui/src/hooks/useTauri.ts` | IPC 封装 |
| `ui/src/i18n/messages.ts` | 中英文文案 |

## Intent coverage

- `acceptance.intent#RuntimeRegistersWhenDaemonStarts`：UI 启动后 Daemon 向 agent-server 发 heartbeat，Runtime 变 online。

## Verification

- [x] `cargo check -p cli-ux` 通过
- [ ] 完整 `cargo build --features ui` 需本机 Xcode license（环境阻塞）

## Notes

- 启动前自动保存 Settings 中的 `server_url` / `runtime_id` 到 `~/.popsicle/agent-runtime.json`。
- 日志追加到 `~/.popsicle/daemon/logs.txt`。
