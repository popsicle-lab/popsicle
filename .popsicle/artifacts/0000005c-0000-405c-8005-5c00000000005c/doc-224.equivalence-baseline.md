...
tests（无 legacy golden）

## Evidence

- [x] `cargo check -p cli-ux` 通过（含 `daemon_start_background` / Tauri command 编译）
- [x] `daemon::parse_start_background_flag` 单元测试通过（lib test；链接受 Xcode license 影响时跳过 bin test）

## Result

增量 UI + CLI `--background` 与既有 `RuntimeRegistersWhenDaemonStarts` 语义一致：后台进程持续 heartbeat。
