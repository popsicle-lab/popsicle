...
id: doc-214
pipeline_run_id: 00000057-0000-4057-8005-57000000000057
status: active
title: PROJ-89 无人值守 orchestrator
version: 1
---

# PROJ-89 无人值守 orchestrator

## Summary

PROJ-89 实现 `run_unattended` orchestrator：`orchestrator.rs` + `orchestrator_parse.rs`；默认启用（`AGENT_RUNTIME_ORCHESTRATOR` 可关）。

## Intent 覆盖表

| 能力 | 实现 | 状态 |
|---|---|---|
| stage 循环 | `run_unattended` | ✅ |
| doc create/check | `execute_doc_create/check` | ✅ |
| stage complete | 按 next 解析 confirm | ✅ |
| issue close | run completed 后 | ✅ |
| 审批暂停/resume | PausedForApproval + confirm 后 resume | ✅ |
| cursor-agent | stage prompt 含 doc_id | ✅ |

## 检查清单

- [x] make check PASS
- [x] agent-daemon 16 tests
