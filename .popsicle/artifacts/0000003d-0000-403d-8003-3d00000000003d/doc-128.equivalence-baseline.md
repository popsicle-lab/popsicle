...
`mode=tests`（无 legacy 源码 diff）

## 门禁

| 门禁 | 结果 | 证据 |
|---|---|---|
| `make check` | pass | fmt + clippy + 全 workspace test |
| 布局迁移单测 | pass | `legacy_self_host_layout_relocates_on_open`、`legacy_tsv_imports_to_sqlite_on_open` |
| dogfood 自动迁移 | pass | `doctor` → `sqlite (.popsicle/state.db)`，`.popsicle/self-host/` 不存在 |
| golden 路径 | pass | baseline 脚本断言更新为 flat 路径 |

## 结论

行为等价：Issue/Run/Doc CLI 与 UI 读写路径变更不影响索引语义；仅磁盘布局扁平化。
