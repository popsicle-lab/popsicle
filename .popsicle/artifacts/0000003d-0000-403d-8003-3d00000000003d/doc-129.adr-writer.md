...
已写入 `products/cli-ux/decisions/adr/ADR-032-workspace-layout-flatten.md`（Accepted）。

## 要点

- Supersedes ADR-013 的 db 路径决策（D-301 正式关闭）
- 目标布局：`.popsicle/{state.db,runs/,artifacts/,modules/,project.yaml}`
- 迁移：`open` 自动 + `admin relocate-workspace`

## 活文档

已更新 `AGENTS.md`、`PROJECT_CONTEXT.md`、`README.md`、`PRODUCT.md`、`ARCHITECTURE.md`、`docs/MIGRATION.md` 与相关 golden 脚本。
