...
status: active
title: telemetry guide 活文档同步
version: 1
---

# telemetry guide 活文档同步
...
一句话结论：PRODUCT / ARCHITECTURE / T-TEL-0002 已指向 `action=guide` 与 module guide 路径；无断链或 orphan。
...
| 来源 | 失效引用 | 类型 |
|---|---|---|
| skill guides（改前） | `products/telemetry/AGENT_TELEMETRY.md` 在非 monorepo 不可解析 | 已修复 → action=guide |
...
| 文件 | 改动 |
|---|---|
| `products/telemetry/PRODUCT.md` | 用户入口增加 action=guide；Roadmap 记 PROJ-75 |
| `products/telemetry/ARCHITECTURE.md` | File Manifest 增 guide.md / tool.yaml / test |
| `products/telemetry/tasks/daily-ops/T-TEL-0002-agent-telemetry-record.md` | 完成路径 step 0 + guide 引用 |
| `products/telemetry/AGENT_TELEMETRY.md` | 指向 module guide 为 Agent 权威 |

## 健康度快照

| 旅程阶段 | Task 数 | 平均行数 | 最久未更新 | 未引用 task |
|---|---|---|---|---|
| daily-ops | 4 | ~70 | 2026-06-26 | 无 |
| admin | 1 | ~59 | 2026-06-26 | 无 |
