...
PROJ-57（T-CU-0017）工作流帮助中心 UI 切流：纯新能力，无 legacy 路径切换；ADR-027 Accepted。

## Gates

| 门禁 | 结果 |
|---|---|
| intent-validate `products/cli-ux/intents` | pass |
| `cargo test -p cli-ux --test workflow_catalog` | pass |
| `cargo build -p cli-ux --features ui` | pass |
| `npm run build` | pass |
| legacy golden ≥5 | **N/A**（新 UI，equivalence 已登记） |
| ADR Accepted | ADR-027 |

## Cutover Actions

- [x] 晋升 ADR-027（Accepted）
- [x] 更新 PRODUCT.md / ARCHITECTURE.md
- [x] acceptance Block 18 + T-CU-0017 task
- [x] 无 migration/traceability 行（非 strangler 项）

## Rollback

移除侧栏 Workflows 路由与 `get_workflow_catalog`；不影响 CLI 主路径。
