...
status: active
title: CADR-001：agent-runtime IDD 专用派活边界
version: 1
---

# CADR-001：agent-runtime IDD 专用派活边界

## Topic

CADR-001：在 `docs/CHARTER.md` 增补 **agent-runtime 为 IDD 专用远程派活**，明确其不是通用工作流/看板平台。来源：PDR-001 product-debate（PROJ-81 doc-181）、ADR-001 §6 CADR 候选。

## Participants

| 角色 | 立场速写 |
|---|---|
| ARCH | 必须走 CADR（触及 L0 charter 产品边界），不可塞进 ADR-001 |
| SEC | IDD-only 降低 Server 被滥用为通用自动化面的攻击面 |
| PERF | 无性能分歧；队列语义限定为 popsicle Issue/Run |
| OPS | charter 明文边界利于运维拒绝 scope creep |
| DEV | PDR-001 已 Proposed，实现 P0–P4 已交付，可 Accept |

用户置信度：5/5

## Phase 1 — 技术问题 + 质量属性（NFR）

- 要解决的问题：charter 未写明 agent-runtime 产品边界，存在被当作通用工作流引擎的风险
- 硬约束：charter 四条铁律 #2（Accepted 决策不可改）→ 新 CADR 追加修订
- 质量属性：可演进性 > 安全 > 性能
- 事实基：PROJ-81 product-debate「非通用工作流平台」；F-2 legacy sync-collab 已砍

## Phase 2 — 方案发散

- **方案 A**（ARCH）：CADR-001 增补 charter「Product Scope Boundaries」节
- **方案 B**（GROWTH 历史）：Multica 桥接通用看板 — 已在 PDR-001 否决
- **方案 C**（PM）：仅 PRODUCT.md 声明 — 不足，未约束 L0
...
| A CADR | ✅ 边界可审计 | ✅ | ✅ | ✅ |
| B Multica | ❌ 双模型 | — | ❌ | ❌ |
| C PRODUCT only | ⚠️ 无 charter 力 | ✅ | ⚠️ | ✅ |

## Phase 4 — 收敛与决策

- ARCH 综合：选方案 A；并行将 PDR-001 从 Proposed → Accepted
- **用户最终决策**：采纳 CADR-001 + PDR-001 Accepted（PROJ-87）

## Decision

在 `docs/decisions/cadr/` 新增 **CADR-001** 修订 charter，限定 agent-runtime 仅服务 IDD pipeline 派活；**PDR-001** 状态改为 Accepted。

## 关键分歧

- charter 是否必须 CADR：ARCH vs PM「PRODUCT 够用」→ CADR 胜出（CONTRIBUTING + charter 自指）

## 用户决策点

- [x] 用户明确要求 CADR-001 / PDR-001 Accepted

## 下游接驳建议

- rfc-writer：CADR-001 正文 + charter diff + PDR-001 Approval 块
- **CADR 候选**：本议题本身即 CADR-001，不走普通 ADR
