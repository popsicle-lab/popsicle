...
status: active
title: CADR-001 + PDR-001 固化报告
version: 1
---

# CADR-001 + PDR-001 固化报告

## Summary

本阶段固化 **CADR-001**（charter 修订）与 **PDR-001 Accepted**。议题触及 L0 charter，**不**产出新 ADR；ADR-001 已 Accepted，按铁律 #2 保持不可变。
...
| Charter 活文档 | `docs/CHARTER.md` § Product Scope Boundaries | 已引用 CADR-001 | ✅ |

## CADR 合规

- [x] 触及 charter Layer Map / 产品边界 → 走 CADR，非普通 ADR
- [x] 未修改 Accepted 的 ADR-001
- [x] Consequences 所列活文档已更新（PRODUCT / ARCHITECTURE / CHARTER）

## Intent Impact 对账

| RFC 行 | 落地 |
|---|---|
| IDD-only 边界 | CADR-001 + CHARTER.md |
| PDR MVP | PDR-001 Accepted |
| contracts | 已在 PROJ-81/82 种子，ADR-001 解锁 |

## Validation

```
popsicle tool run intent-validate path=products/agent-runtime — exit 0
make check — exit 0
```

## 检查清单

- [x] 决策无歧义（IDD-only，非通用工作流）
- [x] Consequences 文件真实存在
- [x] Intent Impact 与 RFC 一致
- [x] Approval 记录完整
