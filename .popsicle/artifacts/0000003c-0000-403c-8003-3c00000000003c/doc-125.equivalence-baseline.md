...
mode=tests：make check 全绿；doctor 无 legacy_artifacts；TSV 打开时自动导入 SQLite。
...
|---|---|---|
| G1 | make check | pass |
| G2 | legacy_tsv_imports_to_sqlite_on_open | pass |
| G3 | admin purge-legacy-workspace | 删除 popsicle.db |
| G4 | doctor storage_backend | sqlite state.db |
