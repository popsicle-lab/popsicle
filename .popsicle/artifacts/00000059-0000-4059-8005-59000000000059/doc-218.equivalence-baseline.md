...
status: active
title: PROJ-91 verify 派活链路验证
version: 1
---

# PROJ-91 verify 派活链路验证
...
|---|---|
| Slice | agent-runtime |
| 验证模式 | tests（无 legacy golden，fix-regression dogfood）|
| Golden 总数 | 0 |
| ✅ pass | 5（手工验收项）|
| ❌ fail | 0 |
| ⚠️ divergence | 0 |
...
门禁：fix-regression verify 采用 targeted tests + 端到端 dogfood 证据，不要求 golden baseline。

## Golden Inventory

- [x] slice 已确认（agent-runtime dispatch）
- [x] ≥5 条 golden 已列出（含输入/legacy/new/比较方式）
- [x] 每条能追溯到 acceptance block 或 api-contracts 行
...
| ID | 描述 | 命令 | 期望 | 结果 |
|---|---|---|---|---|
| G-001 | Server 健康 | `curl /health` | status ok | PASS |
| G-002 | Dispatch 入队 | `POST /v1/dispatch` | accepted queued | PASS |
| G-003 | Daemon claim | daemon logs | claimed task + issue start exit=0 | PASS |
| G-004 | Run mirror | `GET /v1/runs` | PROJ-91 in_progress | PASS |
| G-005 | 编译 | `cargo build -p cli-ux` | exit 0 | PASS |
...
cargo build -p cli-ux — exit 0（含 agent-daemon ureq 404 修复）
```

## Baseline Manifest

本 dogfood 无 legacy golden；数字与上表一致，不写入 baseline.yaml。
...
- [x] equivalence_gate_pass=true（tests 模式，5/5 验收项 PASS）
- [x] 无未登记 divergence
- [x] verify 阶段可完成
