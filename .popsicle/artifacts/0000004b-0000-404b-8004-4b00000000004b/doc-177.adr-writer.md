...
status: active
title: intent goal 追溯硬门禁 ADR 结案
version: 1
---

# intent goal 追溯硬门禁 ADR 结案

## Summary

PROJ-76：`intent-validate` 合并 goal 追溯闸新增 `E_PRODUCT_MISSING_GOALS`；telemetry contracts 补 3 个 goal + realized_by；intent-spec / intent-check skill 与 pipeline 文案对齐。

## 固化检查

- [x] `crates/cli-ux/src/intent_goal_trace.rs` 已合并
- [x] `make check` PASS
- [x] `intent-validate path=products` exit 0

## 解锁动作

- intent-spec-writer / intent-consistency-check 阶段以合并闸为准；零 goal 不可 complete
- 活文档：`docs/PROJECT_CONTEXT.md` §Intent goal 追溯

## Intent Impact 核对

- `products/telemetry/intents/contracts.intent`：新增 3 goal，TelemetrySinkFailOpen 保留
- 其它 product 无破坏性变更
