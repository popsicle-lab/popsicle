...
id: doc-176
pipeline_run_id: 0000004b-0000-404b-8004-4b00000000004b
status: active
title: goal 追溯硬门禁实现覆盖
version: 1
---
...
- [x] target slice 已确认（cli-ux intent_goal_trace + telemetry intents + intent-coder skills）
...
| 指标 | 值 |
|---|---|
| Slice | cli-ux + telemetry intents |
| 已覆盖 | E_PRODUCT_MISSING_GOALS + telemetry 3 goals |
| `make check` | PASS |

一句话结论：合并 goal 追溯闸补齐；telemetry contracts 三 goal + realized_by；skill/pipeline 文案对齐。

## Intent 覆盖表

| Intent / Safety | 层级 | Task ID | 实现位置 | Test | 状态 |
|---|---|---|---|---|---|
| E_PRODUCT_MISSING_GOALS | gate | — | `intent_goal_trace.rs::audit_goal_trace` | `missing_goals_when_contracts_file_expected` | ✅ |
| telemetry goals ×3 | contracts | — | `products/telemetry/intents/contracts.intent` | `merged_products_pass_with_realized_by` | ✅ |

## File Manifest 对账

| 路径 | 状态 |
|---|---|
| `crates/cli-ux/src/intent_goal_trace.rs` | ✅ |
| `products/telemetry/intents/contracts.intent` | ✅ |
| `intent-coder/skills/intent-spec-writer/*` | ✅ |
| `intent-coder/skills/intent-consistency-check/*` | ✅ |
| `intent-coder/pipelines/*-spec*.yaml` | ✅ |
