...
status: active
title: PROJ-92 doc-sync 活文档
version: 1
---

# PROJ-92 doc-sync 活文档

## 同步目标

| target | 文件 | 变更 |
|---|---|---|
| product-header | `products/agent-runtime/PRODUCT.md` | Status→P8；手机 App 入口；Deferred 收敛；Roadmap +P8；关闭 Mobile vs PWA Open Question（Expo）|
| architecture-manifest | `products/agent-runtime/ARCHITECTURE.md` | File Manifest apps/mobile ✅；新增 §P8；CORS 说明 |
| implementation-status | 同上 File Manifest 状态列 | P1→✅ P8 |

## 已修订文件

- [x] `products/agent-runtime/PRODUCT.md`
- [x] `products/agent-runtime/ARCHITECTURE.md`

## 检查清单

- [x] product-header 与实现一致（Expo 非 PWA）
- [x] architecture-manifest 列出 apps/mobile 路径
- [x] 无未填占位符残留于上述活文档
- [x] Decision-Ref 仍指向 CADR-001 / PDR-001
