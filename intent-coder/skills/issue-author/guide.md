...
> **独立 skill**：不进入 `bugfix` / `slice-delivery` 等 pipeline yaml。在
> `popsicle issue create` **之前**由 Agent 执行；人类在 UI 创建 Issue 时可跳过。
>
> **本 guide 是 Issue 创建的唯一入口**：内含 pipeline 决策树、`slice-delivery`
...
在 `issue start` 之前走完本树，避免 spec 未完成就进入 `slice-delivery`。
...
# 2) spec 已覆盖、只写代码时
popsicle issue create --type technical --title "..." --product cli-ux \
  --pipeline slice-delivery --tasks T-CU-0002 \
...
popsicle issue create --type technical \
  ...
  --product cli-ux \
  --pipeline slice-delivery \
  --tasks T-CU-0010,T-CU-0011 \
  --description "实现 T-CU-0010、T-CU-0011 …（须写明每个 task_id）"
```

新旅程（spec 未覆盖）应 `--proposed-task` + `slice-spec`，**不可** `slice-delivery` + proposed：
...
| 时机 | 规则 |
|---|---|
| `issue create` | `--pipeline slice-delivery` 与 `--proposed-task` **不可同用** |
| `issue start` + `slice-delivery` | 禁止含 `proposed` task link |
...
违反时 CLI 返回 `slice-delivery-gate:*` 错误与可执行 `next` 提示。

## 反模式（PROJ-30 / PROJ-35 教训）

| 反模式 | 后果 | 应用 |
|---|---|---|
| 直接 `--pipeline slice-delivery` 写 UI 功能 | 无 PRD/intent 前置，事后补 ADR | 先补 intent/task，或 slice-spec |
...
1. **不要** `issue start` + `slice-spec`（`facts` 面向 legacy 迁移，不是 retro）。
2. **直接**在 `products/<product>/` 写：PDR → task → `acceptance.intent` block。
...
| spec 未覆盖时选 `slice-spec` | 默认 `--type technical` 的 tech-decision |
