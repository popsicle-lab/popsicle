...
> **何时必填**：完成路径 ≥ 4 步，或用户容易走错分支时。
> 先 `popsicle tool run mermaid-diagram action=scaffold type=task-flow title="…"`，再改占位节点。
> 只画 **happy path**；FAIL 分支指向另 task（如 `T-0002`）。写完 `action=validate path=<本文件>`。

Diagram: {简短名称} (flowchart)

```mermaid
flowchart TD
  s1["步骤 1：在 spec 目录"] --> s2["步骤 2：intent check spec.intent"]
  s2 --> s3["步骤 3：5 秒内看到 PASS 或 FAIL"]
  s3 --> ok["成功：终端出现 PASS"]
  s3 --> branch["若 FAIL → 见 T-0002"]
```
