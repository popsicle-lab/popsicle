...
> `references/charter-compliance.md`，以及（如果存在）
> 上游 `product-debate` 的产出 `{slug}.prd-draft.md`、`docs/CHARTER.md`。
...
- legacy 模式：task 中所有「数字 / 模块名 / 风险条目」改写为「[未经事实基验证]」标记，
  质量评分扣 IDD 适配度的相应分数
- greenfield 模式：不因缺 fact report 扣分；引用 Product Brief，超出 brief 的
  市场、成本、风险判断标 `[假设]` / `[待验证]`
...
- legacy 模式：这说明 project-init 还没用 v0.2 的 Scaffolding Manifest 跑过；警告用户
  先跑 project-init 阶段（`migration-bootstrap` pipeline 的 init stage），或手工创建
  5 个旅程目录
- greenfield 模式：允许在 File Manifest 中声明并创建
  `products/{target_product}/tasks/{onboarding,daily-ops,troubleshooting,admin,lifecycle}/`
- 不阻塞本 skill 流程
...
插图前先调 Popsicle tool（技能全文在 tool 的 `guide.md`）：

```bash
popsicle tool run mermaid-diagram action=guide
popsicle tool run mermaid-diagram action=scaffold type=task-flow title="<task 用户原话>"
popsicle tool run mermaid-diagram action=scaffold type=task-relations title="本次 PRD task 链"
popsicle tool run mermaid-diagram action=validate path=products/<product>/tasks/
```

| 产物 | 要求 |
|------|------|
| **prd-overview** | 建议 §「旅程与 Task 关系」；`scaffold type=task-relations` |
| **task 文件** | 完成路径 ≥ 4 步 → **必须**「流程示意」；`scaffold type=task-flow` |
| **PDR** | 一般不画图 |

写入后 `validate`；UI 渲染见 `MarkdownWithMermaid.tsx`。

## 参考资源

### Reference 文件

- **`../../tools/mermaid-diagram/guide.md`** —— Mermaid tool 技能（`tool run mermaid-diagram action=guide`）
...
- **`{slug}.fact-extraction-report.md`** —— legacy 模式的数字 / 模块名引用源
- **`{slug}.project-init-plan.md`** —— legacy 模式的 target product 锁定
- **Product Brief** —— greenfield 模式的目标用户、范围、硬约束依据
