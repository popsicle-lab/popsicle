...
> **Last-Decision-Ref**: ADR-015（Tauri UI MVP+，PROJ-27）
...
| 3 | `cli-ux` | `crates/popsicle-cli/` + `crates/popsicle-core/src/commands/` + `prompt/migrate/admin` 命令族 + **`ui/` Tauri 壳** | **cutover-done** | `slice-3-cli-ux` / `slice-4-ui` | PROJ-27 UI ✓ | ADR-008~015 · DevOps + **Tauri UI MVP+**（feature-gated）· golden 31/31 · **dogfood-usable** |
